...
/** Cuentas con user, filtradas por rol/fuente (para asignar encuestas por cuenta) */
export const filterAccounts = async (req, res) => {
  const body = req.body || {};
  const { rolId, fuente, carrera, periodo, especialidad, periodoAcademico, periodoActivo } = body;
  try {
    const include = [
      {
        model: Account,
        as: "accounts",
        required: true,
        include: [{ model: Roles, as: "roles", through: { attributes: [] }, ...(rolId && { where: { id: rolId } }) }],
      },
    ];
    if (fuente === "matriculas") {
      include.push({ model: Matricula, where: { ...(especialidad && { especialidad }), ...(periodoAcademico && { periodoAcademico }), ...(periodoActivo && { periodoActivo }) } });
    }
    if (fuente === "matrices") {
      include.push({ model: Matriz, where: { ...(carrera && { idCareer: carrera }), ...(periodo && { idPeriod: periodo }) } });
    }
    ...
    for (const u of users) {
      for (const acc of u.accounts || []) {
        if (rolId && !acc.roles?.some((r) => String(r.id) === String(rolId))) continue;
